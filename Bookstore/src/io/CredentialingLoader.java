 ...
 * Write new user information to the Users file (register) Checks entered
 * credentials against existing store credentials (login)
 * 
 * 
...
public final class CredentialingLoader {
	...
	/**
	 * Attempts to match a user name and password with the information stored in
	 ...
	 */
	public static String login(final String theUsername,
			final char[] thePassword) {
		String result = "";
		try (Scanner input = new Scanner(Paths.get(
				R.Strings.IO_FILE_LOCATION + R.Strings.IO_CREDENTIALS_FILE))) {
			while (input.hasNextLine()) {
				final String lineAsString = input.nextLine();
				if (!lineAsString.startsWith(R.Strings.IO_FILE_COMMENT)) {
					final String[] parts = lineAsString
							.split(R.Strings.IO_FILE_DELIMITER);

					final String username = parts[USERNAME.index()];
					if (username.equals(theUsername) && testHashForEquality(
							byteStringToByteArray(parts[SALT.index()]),
	...
	/**
	 * Attempts to add a new user to the credentials file. Note, this method
	 * does not perform validation. Any and all validation must be performed
	 ...
	 * @throws IllegalArgumentException when any of the arguments are non-null
	 *                                  but "empty"
	 */
	public static String register(final String theUsername,
			final char[] thePassword, final String theCampus) {
		String result = "";
		if ("".equals(theUsername) || "".equals(theCampus)
			...
			try (PrintWriter printWriter = new PrintWriter(
					new FileWriter(
							Paths.get(R.Strings.IO_FILE_LOCATION
									+ R.Strings.IO_CREDENTIALS_FILE).toFile(),
							true))) {
	...
	/**
	 * Given a char[] password, and a salt, does the hashing algorithm return
	 * the same hash, in other words, was the same password entered.
	 * 
	...
	private static boolean testHashForEquality(final byte[] theSalt,
			final byte[] theHash, final char[] thePassword) {
	...
	private static byte[] generateHash(final char[] thePassword,
			final byte[] theSalt) {
		byte[] result = null;
		final KeySpec spec = new PBEKeySpec(thePassword, theSalt, 65536,
				SALT_SIZE * 8);
